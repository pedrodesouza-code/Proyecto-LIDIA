...
hasta disponer de una fuente aprobada y trazable del proyecto.

## Contenido Implementado

- `implementation/sql/ddl/`: esquemas `staging`, `dw`, `audit`, roles, modelo
  estrella, indices, vistas, migracion legacy segura y PostGIS opcional FIRMS.
- `implementation/etl/`: extractores por fuente, validacion, rechazos,
  promociones al DW y CDC mediante clave natural mas hash SHA-256.
- `implementation/nosql/`: schemas y consultas MongoDB para metadata,
  rechazos y snapshots; PostgreSQL sigue siendo el Data Warehouse.
- `implementation/dashboard/streamlit_app.py`: dashboard conectado a vistas SQL.
- `implementation/tests/`: calidad, idempotencia y CDC con resultados numericos.
...
MongoDB complementa al DW con documentos de ejecucion, rechazos y snapshots
FIRMS. El contrato esta en `implementation/nosql/mongo_schema.json`; no
...
Para validacion local se provee `implementation/docker/docker-compose.yml`.
...
esta documentado en `implementation/docker/README.md`.
