...
-- Extensión geoespacial para puntos FIRMS y consultas espaciales.
-- Requiere imagen/instalación PostgreSQL con PostGIS disponible.
...
CREATE TABLE IF NOT EXISTS focos_calor (
    ...
    confianza_raw       VARCHAR(10),
    confianza_num       SMALLINT     CHECK (confianza_num BETWEEN 0 AND 100),
    satelite            VARCHAR(20),
...
COMMENT ON COLUMN focos_calor.geom IS 'Geometría puntual WGS84 generada desde longitud/latitud del foco FIRMS';
COMMENT ON COLUMN focos_calor.potencia_radiativa IS 'Fire Radiative Power en megawatts (MW)';
COMMENT ON COLUMN focos_calor.confianza_num IS 'Confianza normalizada: VIIRS l/n/h -> 1/2/3; MODIS conserva escala 0-100';
