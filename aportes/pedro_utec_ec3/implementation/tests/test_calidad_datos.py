...
class TestConsistencia:
    ...
    def test_firms_coordenadas_en_sudamerica(self, df_firms):
        if "latitud" not in df_firms.columns:
            pytest.skip()
        fuera = int(
            (~df_firms["latitud"].between(-56.0, 13.0) |
             ~df_firms["longitud"].between(-82.0, -34.0)).sum()
        )
        metricas = {"fuera_de_sudamerica": fuera, "total": len(df_firms)}
        estado = "PASS" if fuera == 0 else "FAIL"
        msg = f"{fuera} focos fuera del bounding box de Sudamérica"
...
class TestValidez:
    ...
    def test_firms_confianza_valida(self, df_firms):
        if "confianza_num" not in df_firms.columns:
            pytest.skip()
        valores = pd.to_numeric(df_firms["confianza_num"], errors="coerce").dropna()
        invalidos = int((~valores.between(0, 100)).sum())
        metricas = {"invalidos": invalidos, "rango": "[0, 100]"}
        estado = "PASS" if invalidos == 0 else "FAIL"
        msg = f"{invalidos} valores de confianza fuera de rango [0,100]"
        _guardar_resultado("validez_firms_confianza", "calidad", estado, metricas, msg)
        assert invalidos == 0, msg

    def test_firms_frp_no_negativo(self, df_firms):
        if "potencia_radiativa" not in df_firms.columns:
            pytest.skip()
        valores = pd.to_numeric(df_firms["potencia_radiativa"], errors="coerce").dropna()
        negativos = int((valores < 0).sum())
        metricas = {"negativos": negativos}
        estado = "PASS" if negativos == 0 else "FAIL"
        msg = f"{negativos} valores FRP negativos"
        _guardar_resultado("validez_firms_frp_no_negativo", "calidad", estado, metricas, msg)
        assert negativos == 0, msg
