...
def _normalize_record(source: str, raw: dict[str, Any]) -> dict[str, Any]:
    ...
    if source == "FIRMS":
        ...
        if not fecha or not location:
            raise ValueError(f"{source} requiere fecha y ubicacion/estacion")
        natural = "|".join([source, fecha, country, location])
        record = {
            "natural_key": natural, "fuente": source, "fecha": fecha, "pais_codigo": country,
            "ubicacion": location, "latitud": _number(_value(raw, "latitud", "latitude", "lat")),
            ...
            "precipitacion_mm": _number(_value(raw, "precipitacion_mm", "precipitation_sum", "rain")),
        }
