...
TABLES = {
    ...
    "METEO": ("stg_meteo", ["record_hash", "natural_key", "fuente", "fecha", "pais_codigo", "ubicacion", "latitud", "longitud", "temperatura_c", "humedad_pct", "viento_kmh", "precipitacion_mm", "raw_payload"]),
    "FORECAST": ("stg_meteo", ["record_hash", "natural_key", "fuente", "fecha", "pais_codigo", "ubicacion", "latitud", "longitud", "temperatura_c", "humedad_pct", "viento_kmh", "precipitacion_mm", "raw_payload"]),
    "INUMET": ("stg_meteo", ["record_hash", "natural_key", "fuente", "fecha", "pais_codigo", "ubicacion", "latitud", "longitud", "temperatura_c", "humedad_pct", "viento_kmh", "precipitacion_mm", "raw_payload"]),
...
def _promote(cur, source: str) -> None:
    if source == "FIRMS":
        ...
        cur.execute("""INSERT INTO dw.dim_ubicacion (pais_codigo,pais_nombre,ubicacion,latitud,longitud)
            ...
            ON CONFLICT (pais_codigo,latitud,longitud) DO NOTHING""", (source,))
        if source == "INUMET":
            cur.execute("""INSERT INTO dw.dim_estacion_meteorologica
                (codigo_estacion,nombre,latitud,longitud)
                SELECT DISTINCT ubicacion, ubicacion, latitud, longitud FROM staging.stg_meteo
                WHERE fuente='INUMET' AND latitud IS NOT NULL AND longitud IS NOT NULL
                ON CONFLICT (codigo_estacion) DO UPDATE SET latitud=EXCLUDED.latitud, longitud=EXCLUDED.longitud""")
        cur.execute("""INSERT INTO dw.dim_clima (fecha_id,ubicacion_id,estacion_id,fuente,temperatura_c,humedad_pct,viento_kmh)
            SELECT d.fecha_id,u.ubicacion_id,e.estacion_id,s.fuente,s.temperatura_c,s.humedad_pct,s.viento_kmh
            ...
            ON CONFLICT (fecha_id,ubicacion_id,fuente) DO UPDATE SET estacion_id=EXCLUDED.estacion_id,
              temperatura_c=EXCLUDED.temperatura_c, humedad_pct=EXCLUDED.humedad_pct, viento_kmh=EXCLUDED.viento_kmh""", (source,))
        ...
        cur.execute("""UPDATE dw.fact_incendio f SET precipitacion_id=p.precipitacion_id, actualizado_en=NOW()
            FROM dw.dim_precipitacion p JOIN dw.dim_fecha dp ON dp.fecha_id=p.fecha_id
            JOIN dw.dim_fecha df ON df.fecha_id=f.fecha_id
            WHERE p.ubicacion_id=f.ubicacion_id AND dp.anio=df.anio AND dp.mes=df.mes""")
    elif source == "MODIS":
        ...
        cur.execute("""UPDATE dw.fact_incendio f SET cobertura_id=c.cobertura_id, actualizado_en=NOW()
            FROM dw.dim_cobertura_vegetal c JOIN dw.dim_fecha d ON d.fecha_id=f.fecha_id
            WHERE c.ubicacion_id=f.ubicacion_id AND c.anio=d.anio""")
