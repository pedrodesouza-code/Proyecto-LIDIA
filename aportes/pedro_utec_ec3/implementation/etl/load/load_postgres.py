...
def cargar_focos_calor(df: pd.DataFrame) -> dict[str, int]:
    ...
    def _to_tuple(row: pd.Series) -> tuple:
        ...
        return (
            ...
            _safe_val(row.get("tipo_foco")),
            "FIRMS",
        )

    SQL_TEMPLATE = """
        INSERT INTO focos_calor
            (fecha_adq, hora_adq_hhmm, latitud, longitud, geom, pais,
             potencia_radiativa, confianza_raw, confianza_num,
             satelite, instrumento, dia_noche, es_diurno,
             brillo_ti4, brillo_ti5, tipo_foco, fuente)
        VALUES %s
        ...
        DO UPDATE SET
            ...
            geom               = EXCLUDED.geom
        WHERE
            focos_calor.potencia_radiativa IS DISTINCT FROM EXCLUDED.potencia_radiativa
            OR focos_calor.confianza_num IS DISTINCT FROM EXCLUDED.confianza_num
            OR focos_calor.pais IS DISTINCT FROM EXCLUDED.pais
    """
        ...
        try:
            ...
            with conn2.cursor() as cur:
                try:
                    ...
                    conn2.rollback()
                    cur.execute("REFRESH MATERIALIZED VIEW mv_focos_por_pais_mes")
            conn2.commit()
            conn2.close()
